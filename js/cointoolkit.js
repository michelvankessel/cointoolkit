$(document).ready(function() {
	...
	async function getLedgerSignatures(currenttransaction, hashType, callback) {
		try {
			...
			var paramsSplit = {transactionHex=currenttransaction.serialize(), isSegwitSupported=false, hasTimestamp=isPeercoin, false, additionals=["peercoin"]}
			...
			for (var i = 0; i < currenttransaction.ins.length; i++) {
				...
				// todo replace !isPeercoin with proper segwit support flag from coinjs params
					var paramsSplit = {transactionHex=result[0], isSegwitSupported=false, hasTimestamp=isPeercoin, false, additionals=["peercoin"]}
					...
					if (inputs.length == currenttransaction.ins.length) {
						...
						if (currenttransaction.ins[0].script.buffer.slice(-1) == coinjs.opcode.OP_CHECKMULTISIG) {
							...
							var params = {inputs:inputs, associatedKeysets:paths, outputScriptHex:outputsBuffer, initialTimestamp:timeStamp, sigHashType: hashType,  additionals: ["peercoin"]};
							...
							$.each(result, function(idx,itm) {
								...
								if (currenttransaction.signmultisig(idx,undefined,Crypto.util.hexToBytes(hashType.toString(16)),signature)) {
									success=true;
									}
								});
								if (success) {
